...
@section('title', 'Edit Data Progress Status')

@section('content')
<div class="space-y-4">
    <!-- Back Button -->
    <a href="{{route('admin.lokasi.show', $status->lokasi_id)}}" class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md transition duration-200">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 class="text-lg font-semibold text-gray-800">Edit Data Progress Lokasi</h3>
        </div>
        
        <!-- Card Body -->
        <div class="p-6">
            <form action="{{route('admin.status-lokasi.update', $status->id)}}" method="POST" class="space-y-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    >        
                            <option value="pending" @if (old('status',$status->status) == 'pending') selected @endif
                                >Pending
                            </option>
                            <option value="approved" @if (old('status',$status->status) == 'approved') selected @endif
                                >Approved
                            </option>
                            <option value="rejected" @if (old('status',$status->status) == 'rejected') selected @endif
                                >Rejected
                            </option>
                            <option value="done" @if (old('status',$status->status) == 'done') selected @endif
                                >Done
                            </option>
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="catatan" name="catatan" value="{{old('catatan', $status->catatan)}}"
